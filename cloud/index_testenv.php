...
// 检测权限
$safeMsg = array();
if(TestExecuteable(DEDEROOT.'/yunteng_cc_data',$cfg_basehost) || TestExecuteable(DEDEROOT.'/yunteng_cc_images',$cfg_basehost))
{
	$helpurl = "http://www.yunteng.cc";
	$safeMsg[] = '目前yunteng_cc_data、yunteng_cc_images有执行.php权限，非常危险，需要立即取消目录的执行权限！
...
{
	$safeMsg[] = '强烈建议yunteng_cc_data/common.inc.php文件属性设置为644（Linux/Unix）或只读（NT）；';
}
$rs = TestAdminPWD();
if($rs < 0)
{
...
if(PostHost($cfg_basehost.'/yunteng_cc_data/admin/ver.txt') === @file_get_contents(DEDEDATA.'/admin/ver.txt'))
{
	$helpurl = 'http://www.yunteng.cc';
